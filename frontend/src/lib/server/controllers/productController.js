...
const createProduct = async (req, res) => {
  try {
    ...
    if (stock < 0) return res.status(400).json({ error: 'Stock must be non-negative' });
    ...
    res.status(201).json({ message: 'Product created successfully', id: Number(result.lastInsertRowid) });
  } catch (error) {
